...
STAMPED complements formal computational workflow guidelines (e.g., FAIR4RS\cite{barker_introducing_2022}, RO-Crate) and facilitates the transition toward comprehensive workflow automation.
...
As always, self-containing all data might be undesired or prohibitive due to size or other concerns. Extensions such as \texttt{git-annex}, \texttt{DataLad}, \texttt{DVC}, and \texttt{Git LFS} strike the balance between the two properties by tracking metadata regarding data obtainability instead, e.g., from which URLs on remote servers act as a source, and providing actionable interfaces to obtain the data when necessary.

This overlap is not incidental — it reflects the interdependence of the STAMPED properties themselves.
Achieving Ephemerality, for instance, inherently exercises Portability and Actionability: a disposable environment must be reconstituted from explicit specifications, and its setup must be automated end-to-end.
...
A comprehensive up-to-date review of existing tools is out of the scope for this formalization paper and would quickly become outdated.
To fill that niche, we initiated a satellite project with a website to describe tools and examples, and  characterize them in terms of STAMPED and FAIR principles [cite https://myyoda.github.io/principles-examples/stamped_principles/].

STAMPED properties are tool-agnostic — any system that moves a property from documented to executable moves the research object further along the actionability spectrum — but certain tool categories recur across multiple properties, reinforcing that a well-chosen toolchain can address several dimensions at once.
Researchers should select tools based on their domain, infrastructure, and team familiarity, recognizing that the tools listed here are illustrative rather than prescriptive: what matters is that the underlying property is satisfied, not which specific tool achieves it.
...
\hline
\textbf{P — Portability} &
Docker, Singularity, Nix, conda, \texttt{pip} (\texttt{pyproject.toml}, \texttt{requirements.txt}), Node &
