...
The FAIR (Findable, Accessible, Interoperable, Reusable) principles\cite{wilkinson_fair_2016} and FAIR4RS guidelines\cite{barker_introducing_2022} for research software provide structured guidance to ensure that digital objects are managed in ways that support reuse and reproducibility.
Here we adopt the term \textbf{research object} to be a collection of data, code, and metadata that together represent the complete unit of research output.
Reproducing the computational results captured by a research object requires specific versions of data, code, and computational environments, as well as detailed provenance of how they were used together.
...
The Workflow Community Initiative’s FAIR Workflows Working Group (WCI-FW)\cite{wilkinson_applying_2025} defines a \textbf{workflow} as ``the formal specification of the data flow and execution control between executable components, the expected datasets, and parameter files.’’
WCI-FW systematically mapped existing FAIR and FAIR4RS principles onto computational workflows, defining metrics for workflow FAIRness and recommendations for workflow developers and systems.
...
Here we identify and codify these practices as the STAMPED properties (Self-contained, Tracked, Actionable, Modular, Portable, Ephemeral, and Distributable), originating from the YODA Principles\cite{hanke_yoda_2018},
...
A researcher who uses version control for an analysis script and documents how to run it is already practicing Self-containment, Tracking, and Actionability at their practical minimum.
As complexity grows---multi-institutional collaborations with federated datasets, heterogeneous workflows spanning platforms, long-term reproducibility requirements---the same properties guide researchers toward more rigorous practices, scaling from practical minimums toward aspirational ideals without requiring wholesale adoption of new tools.
STAMPED is not a compliance threshold but an incremental framework: any research object can be evaluated against it, and any improvement along any dimension is meaningful.
To assist with such incremental improvement, for each STAMPED property we provide compliance checklists with different strengths of the requirements.
As such, STAMPED complements formal computational workflow guidelines (e.g., FAIR4RS\cite{barker_introducing_2022}, RO-Crate) and facilitates the transition toward comprehensive workflow automation.
