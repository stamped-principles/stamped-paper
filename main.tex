...
A research object that is self-contained, tracked, and modular may still fail to run on a different machine if it is silently affected by any inherited host state.
...
Ephemerality eliminates this problem by construction; a disposable environment built from tracked specifications cannot carry any undocumented state, so that successful execution exercises Self-containment, Actionability, and specification completeness.
...
Tracked and self-contained research objects let a researcher inspect which model, prompt, and decision led to a particular result, implement ``hand over'' between humans and agents alike and let subsequent actor re-enter any prior state and continue from it without rediscovering context.
