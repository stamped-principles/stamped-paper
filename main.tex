...
\begin{itemize}[label=$\triangleright$]
    ...
    \end{itemize}
    \item \textbf{Tracking (T.1)+(T.3)}: Persistent content identification MUST be recorded for all components. Provenance of all modifications MUST be recorded.
    \begin{itemize}[label=$\square$]
    ...
    \end{itemize}
    \item \textbf{Distributability (D.1)}: All referenced modules and components MUST be persistently retrievable by others.
    \begin{itemize}[label=$\square$]
...
\subsection{Future Directions: Research in an AI Era}

AI agents are increasingly embedded in research workflows, from generating analysis code to orchestrating multi-step pipelines.
As these tools mature, the structural properties of research objects will determine whether AI-assisted research remains reproducible or drifts toward opaque, unreproducible automation.
STAMPED properties already support this transition: self-contained research objects give agents a well-defined workspace, tracked histories make agent contributions auditable, and actionable documentation provides instructions that both humans and machines can follow.
However, several dimensions of the framework may need to evolve to fully address the challenges AI introduces.
...
In many cases, users interact with models through platforms without knowing which model version produced their results.
Even when models are nominally versioned, the degree of version control varies widely: Hugging Face provides commit-level pinning, while commercial APIs may silently update the model behind a fixed endpoint.
Self-containment (S) could extend naturally to address this: treating models as tracked components of the research object, just as data and code are today.
Practical mechanisms exist for open models, such as pinned revisions on Hugging Face or storing the whole model via git-annex, and models managed in this way would immediately offer greater value for use in scientific workflows.
A related challenge remains: tracking what training data and procedures produced a given model is an open problem that intersects questions of copyright, benchmarking, and provenance, largely beyond what STAMPED can prescribe today.
...
In principle, tracking (T) can extend to cover agent actions --- for example, wrapping an agent invocation in a provenance-capturing command such as \texttt{datalad run} to record the model, prompt, and resulting changes alongside the output.
Some of this provenance is already captured organically through git commits authored by agents, session transcripts, and co-authorship metadata, but no standards exist for what constitutes sufficient provenance for an AI-driven modification.
A practical tension exists: agent sessions can produce enormous transcripts, and including them wholesale may dilute the value of more targeted provenance records.
Determining what to capture (and at what granularity) remains an open question.
The results will not be bitwise reproducible, since language model outputs are inherently stochastic, but exact reproducibility is rarely the goal; what matters is that the reasoning and decisions are recoverable.
...
Actionability (A) has traditionally meant that a human can execute a documented workflow.
AI agents introduce a second consumer: machines that parse prose instructions to determine what actions to take.
Configuration files such as CLAUDE.md, README-driven agent workflows, and structured project metadata already serve this role, making STAMPED-organized research objects more accessible to automated tools.
This represents a practical test of non-executable, prose-based instructions --- currently less reliable than deterministic execution, but an increasingly important interface as agent-driven research becomes routine.

\paragraph{Ephemeral software.}
Ephemerality (E) currently applies to compute environments: containers and virtual machines that are created, used, and discarded.
The rise of AI-generated code extends this property to software itself.
When an agent can regenerate analysis code from a method specification, the specific implementation becomes disposable.
The specification accumulates real-world experience, including use cases, bugs, and edge cases, and robustness emerges from this accumulated knowledge rather than from any particular codebase.
This reframes reproducibility: rather than preserving a fixed software stack, researchers preserve the knowledge and constraints from which working code can be derived.
Specification-driven approaches such as SpecKit point toward this future, where STAMPED properties would apply to the specifications themselves --- self-contained, tracked, and actionable descriptions from which software is generated on demand.
STAMPED's emphasis on tracked, self-contained method descriptions provides the foundation that makes ephemeral software viable rather than reckless.
