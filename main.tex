...
Scientific claims now rest on stacks of code, data, and environments; yet the provenance of how they are used together is routinely incomplete, scattered, or lost, which undermines rigor, reproducibility, reusability, and efficiency.
Researchers across domains have converged toward analogous patterns, and tooling has matured to meet them.
What is still missing is a shared vocabulary for how a research object, a complete unit of data, code, and metadata, should be structured and managed so it can be re-executed, extended, and redistributed; FAIR and FAIR4RS apply to digital objects in general, but not to this operational layer.
Building on the YODA and VAMP frameworks, we formalize seven principles: Self-contained, Tracked, Actionable, Modular, Portable, Ephemeral, and Distributable (STAMPED).
Together they guide how researchers produce and evaluate research objects so the resulting work is rigorous, reproducible, and reusable.
We frame each principle as a spectrum, so adoption is incremental and starts from existing practice.
