...
The FAIR principles (Findable, Accessible, Interoperable, and Reusable)\cite{wilkinson_fair_2016} provide structured guidance for digital objects, and the FAIR4RS guidelines\cite{barker_introducing_2022} extend these principles to research software.
The Workflow Community Initiative (WCI-FW)\cite{wilkinson_applying_2025} mapped these principles onto computational workflows but explicitly chose not to define new ones, instead treating workflows as hybrid data and software objects.
...
For example, persistent identifiers serve both Findability and the ability to track components over time, but accent different aspects: FAIR emphasizes discovery and interoperability across repositories, while STAMPED emphasizes improving the operational maturity of scientific workflows in day-to-day research practice.

Many researchers already employ practices that address these operational properties, such as using version control, adopting containerized environments, organizing structured project directories, and documenting analysis steps. % TODO: It is unclear what the operational properties are at this point, so it can be confusing to the user.
Yet the research community lacks a shared vocabulary for referring to these practices, evaluating how thoroughly they are applied, and communicating the evaluation to collaborators and reviewers.
Here we provide that vocabulary: the STAMPED properties (Self-contained, Tracked, Actionable, Modular, Portable, Ephemeral, and Distributable), derived from the YODA Principles\cite{hanke_yoda_2018}.
The STAMPED properties provide a framework for evaluating how thoroughly these practices are applied and thus the operational maturity of a research object.
Each STAMPED property is conceived as a spectrum from a practical baseline, reflecting common existing practices, to an aspirational ideal of fully realized operational maturity, rather than a binary notion of compliance.
For example, a research object that has all components under one project directory, includes version-controlled analysis scripts, and contains documentation on how to run them already meets the practical baseline for Self-containment, Tracking, and Actionability.
As complexity increases, such as in multi-institutional collaborations with heterogeneous workflows, these properties guide the transition toward more rigorous, reproducible practices without requiring wholesale adoption of new tools.
