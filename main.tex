...
The items containing keywords "MUST", "SHALL", "SHOULD", and "MAY" are to be interpreted as described in RFC 2119. % TODO: cite RFC
...
Similarly, version control systems such as \texttt{Git} underpin both Tracking (content-addressed identification and change history) and Self-containment (gathering all components under a single root), while extensions such as \texttt{git-annex}, \texttt{DataLad}, \texttt{DVC}, and \texttt{Git LFS} further extend these properties to large binary data.
...
\texttt{git-annex} supports many remote types (SSH, S3, HTTP, local drives, offline USB archives), making it well-suited to multi-institutional collaboration and long-term archival where data sovereignty or offline access is required.
...
\begin{table}[ht]
\centering
\caption{Enabling tools for each STAMPED property.}
...
\hline
\textbf{T — Tracking} &
\texttt{Git}, \texttt{git-annex}, DataLad, DVC, Git LFS &
Version control for code and data; content-addressed identification; provenance recording (e.g., \texttt{datalad run} for programmatic provenance capture) \\
\hline
