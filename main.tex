...
FAIR addresses how digital objects should be described so they can be found, accessed, and reused; STAMPED addresses a complementary question: how a research object should be organized and managed so it can actually be re-executed, extended, and redistributed.
...
\subsection{Convergent evolution of empirical scientific practice}
...
What follows is not a comprehensive review but a tracing of that convergence, organized chronologically, to show that the properties STAMPED formalizes reflect durable responses to recurrent challenges rather than arbitrary design choices.
...
The YODA principles\cite{hanke_yoda_2018} combined these insights into organizational rules from which STAMPED directly descends.
...
Galaxy\cite{afgan_galaxy_2018} (first released 2005), Code~Ocean (2017), brainlife.io\cite{hayashi_brainlifeio_2024}, a non-exhaustive list, provided turnkey reproducibility services, each implementing separation of code, data, and environment as a first-class abstraction---reflecting Self-containment~(S), Actionability~(A), and Portability~(P).
...
Each framework constrains where raw data, derived outputs, code, and documentation reside, making projects navigable by \emph{convention} rather than documentation alone.
