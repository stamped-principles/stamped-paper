...
The directions below identify open fronts where the framework's vocabulary will need to stretch, and where complementary work by adjacent communities will define how STAMPED will assist researchers in the coming decade.

\subsubsection{Specification-centric research objects}
...
STAMPED does not prescribe a serialization for provenance records, and in practice researchers rely on whatever their tooling emits.
Convergence among community standards --- W3C PROV\cite{w3c_provenance_working_group_prov-overview_2013}, RO-Crate\cite{peroni_packaging_2022}, and domain-specific provenance such as BIDS-Prov (BEP028)\cite{bids_community_bep028_2025} --- would let STAMPED research objects carry portable provenance across toolchains without lock-in.
This is less a change to STAMPED itself than a call on these communities and relation to FAIR principles: Tracking (T) becomes substantially more useful when provenance records follow standards and can be exported, compared, and re-interpreted by tools other than the one that generated them.
...
STAMPED is only as useful as the fraction of students and early-career researchers who encounter it during training.
...
Curricular integration with open-science and neuroimaging training programs (ReproNim, Brainhack) would close the gap between practitioners who do these things by habit and those who can name what they are doing and evaluate it against a shared rubric.
YODA principles have already entered the curricula of several such programs, and we expect their references to migrate toward the more formally articulated STAMPED vocabulary; continued community outreach, including recorded talks such as Cody Baker's introduction to STAMPED\cite{baker_guidelines_2026}, supports that transition.
...
STAMPED properties already carry most of what is needed --- self-contained workspaces give agents a well-defined boundary, tracked histories make agent contributions auditable and contribute to their ``memory'', and actionable documentation serves humans and machines alike --- but each property needs an AI-era extension.
