...
Yet the records of how they are used together is often incomplete, scattered, or lost.
This undermines rigor, reproducibility, reusability, and efficiency.
What is still missing is a shared vocabulary for how a research object as a complete unit of data, code, and metadata, should be structured and managed so it can be re-executed, extended, and redistributed.
FAIR and FAIR4RS apply to digital objects in general, but not to this operational layer.
Building on previous frameworks, we formalize seven principles: Self-contained, Tracked, Actionable, Modular, Portable, Ephemeral, and Distributable (STAMPED).
Together they give researchers guidelines for building and assessing research objects that others can trust, rerun, and build on.
We frame each principle as a spectrum so that adoption is incremental and starts from existing practice.
The independent convergence across disciplines toward these same principles over decades is evidence they name recurrent patterns, not just another framework.
We back each principle with normative requirements, an interactive checklist, and a map of enabling tools.
With maturing conventions, increasingly capable tooling, and the arrival of AI agents, rigor, reproducibility, reusability, and efficiency are within closer reach than ever.
This shared vocabulary augments all of them, giving researchers, collaborators, reviewers, and the agents working with them a common language, concrete goals, and an aligned direction.
