...
During development, researchers routinely modify their environment---installing packages, adjusting configurations, setting variables---and each change is individually small but collectively difficult to document, time-consuming to verify, and hard to confirm as a complete specification.
The result is environment drift: an environment that works but may not be reconstructable from its specification alone, where verifying completeness is costly and undocumented steps can go unnoticed when institutional knowledge silently fills the gaps.
...
Running in a fresh environment on the same host demonstrates that specifications are sufficient; running on a different host demonstrates that they are not coupled to a specific system.
