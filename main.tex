...
\author{Austin Macdonald, Cody C. Baker, Yaroslav O. Halchenko, Isaac To}
\date{March 2025}
...
Computational research depends on specific versions of datasets, software, and computational environments, together with a record of how they were combined to produce results.
We use the term \textbf{research object} to denote a collection of data, code, and metadata that together represent a unit of research output.
Currently, components of a research object are most often managed separately---code in one repository, data on a shared drive, environment setup in a wiki page, provenance in a lab notebook or nowhere at all---and this fragmentation undermines rigor, reproducibility, reusability, and efficiency.
...
These frameworks address \emph{what} should be shared and how it should be described, but not the \emph{operational} properties---self-containment, provenance tracking, portability, environment management---that determine whether a research object can be reliably executed, reused, and shared.
...
Here we provide that vocabulary: the STAMPED properties (Self-contained, Tracked, Actionable, Modular, Portable, Ephemeral, and Distributable), originating from the YODA Principles\cite{hanke_yoda_2018}, which describe the operational maturity of a computational research object.
...
A researcher who version-controls analysis scripts, documents how to run them, and stores everything under one project directory is already practicing Self-containment, Tracking, and Actionability at their practical floor.
...
This spectrum means STAMPED applies across the full range of research complexity.
STAMPED provides a vocabulary and spectrum-based framework for describing, evaluating, and incrementally improving the operational properties of computational research objects toward enhanced rigor, reproducibility, reusability, and efficiency.
...
These are often managed separately---versions in one system, provenance in a lab notebook or nowhere at all---and this fragmentation makes prior work hard to verify or reproduce.

The spectrum of Tracking begins with content-addressed version control.
Rather than relying on semantic version labels, content-addressed systems identify each state by a cryptographic checksum of its contents.
...
Traditional version control is designed for source code, but research objects often include large binary datasets, container images, and other assets that do not fit this model.
...
Without clear boundaries between the conceptual components of a research object, it is difficult to manage them independently.
...
At a minimum, a research object might separate analysis code, input datasets, computational environments, and results into distinct directories, making it navigable and maintainable.
...
Some degree of coupling to the host is unavoidable---paths, resource limits, platform-specific flags.
...
The result is environment drift: an environment that works but may not be reconstructable from its specification alone, where undocumented steps can go unnoticed when institutional knowledge silently fills the gaps.
...
To fill that niche, we initiated a satellite project with a website to describe tools and examples, and characterize them in terms of STAMPED and FAIR principles [cite https://myyoda.github.io/principles-examples/stamped\_principles/].
...
% TODO: AI and Actionability — as AI tools improve, the gap between documented and executable narrows, but executable specifications remain more reliable. Discuss implications for the actionability spectrum.
